...
Uruchom po ręcznym stworzeniu głównego folderu i podfolderów pracowni.
"""
...
def short_project_name(folder_name: str, meta: dict, workshop_code: str) -> str:
...
def create_inf_template(path: Path, meta: dict, workshop_code: str, project_name: str) -> None:
    try:
...
def run_setup(root: Path, log) -> bool:
    ...
    for cat_dir in sorted(root.iterdir()):
        ...
        for workshop_dir in sorted(cat_dir.iterdir()):
            ...
            inf_name = f"{meta['surname_initial']}_{w_code}_{meta['year']}_{meta['semester']}_{proj}_INF.txt"
    ...
    log("    Następnie uzupełnij pliki _INF.txt i uruchom  2_ASP_Archiwum.")
    return True
...
class SetupApp(tk.Tk):
    ...
    def _build_ui(self):
        # ── Header ────────────────────────────────────────────────────────────
        header = tk.Frame(self, bg=BG, pady=24)
        ...
        # ── Wybór folderu ─────────────────────────────────────────────────────
        pick_frame = tk.Frame(self, bg=BG, pady=20)
        ...
        row = tk.Frame(pick_frame, bg=BG, pady=8)
        row.pack(fill="x")
        ...
        # ── Instrukcja ────────────────────────────────────────────────────────
        info = tk.Frame(self, bg=BG_CARD, padx=16, pady=14)
        ...
        steps = [
            ...
            "4.  Wybierz folder główny powyżej i kliknij  Uruchom Setup",
        ]
        for s in steps:
            tk.Label(info, text=s, font=FONT_SUB, fg=TEXT_DIM, bg=BG_CARD,
        ...
        self._log_box.pack(fill="both", expand=True, padx=32, pady=(4, 0))
